...
from assignments.models import assignment, Pdf, Video, Test, Test_question, user_progress_video, user_progress_pdf, AComment, pdfAComment
...
def pdfView(request, id):
    ...
    done = True if request.user.read_assignment_pdf.filter(
        Pdf=p) else False
    data = {
        'done': done,
        'pdf': p,
    }
    return render(request, 'assignments/pdf.html', data)


def AssignmentPdfComments(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            # get post object
            ...
            # if parent_id has been submitted get parent_obj id
            if parent_id:
                parent_obj = pdfAComment.objects.get(id=parent_id)

            doubt = True if request.POST['doubt'] == 'true' else False

            pdfAComment.objects.create(
...
def likePdfComment(request):
    if request.user.is_authenticated:
        comment = pdfAComment.objects.get(id=request.POST.get('id'))
        if comment.likes.filter(id=request.user.id).exists():
            comment.likes.remove(request.user)
            if request.user.admin or request.user.is_staff:
                data = {
...
def deletePdfComment(request):
    if request.user.is_authenticated:
        comment = pdfAComment.objects.filter(id=request.POST['id'])
        id = comment[0].id
        pd = comment[0].Pdf
        if request.user.user_type == "Student":
            if comment[0].Author != request.user:
        ...
        data['parent_id'] = id
        data['body'] = render_to_string('video/discussions.html', {'comments': pdfAComment.objects.filter(
...
def updatePdfComment(request):
    if request.user.is_authenticated:
        comment = pdfAComment.objects.get(id=request.POST['id'])
        if 'resolved' in request.POST:
            comment.resolved = not comment.resolved
        else:
            comment.body = request.POST['body']
